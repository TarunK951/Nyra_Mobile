...
export const SPRING = {
    ...
    gentle: { tension: 180, friction: 18, useNativeDriver: true },
    bouncy: { tension: 240, friction: 12, useNativeDriver: true },
    modal: { tension: 200, friction: 20, useNativeDriver: true },
};
...
export const usePulse = (min = 0.85, max = 1) => {
    ...
    useEffect(() => {
        Animated.loop(
            Animated.sequence([
                Animated.timing(scale, { toValue: max, duration: 900, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
                Animated.timing(scale, { toValue: min, duration: 900, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
            ])
        ).start();
    }, []);
...
export const useShimmer = () => {
    ...
    useEffect(() => {
        Animated.loop(
            Animated.sequence([
                Animated.timing(opacity, { toValue: 0.9, duration: 1600, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
                Animated.timing(opacity, { toValue: 0.4, duration: 1600, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
            ])
        ).start();
    }, []);
